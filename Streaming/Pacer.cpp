...
void Pacer::init(const std::shared_ptr<DX::DeviceResources> &res, int streamFps, double refreshRate) {
	...
	m_FrameCadence.setDisplayHz(m_RefreshRate > 0.0 ? m_RefreshRate : 60.0);
	if (streamFps > 0) {
		m_FrameCadence.setStreamFpsOverride(static_cast<double>(streamFps));
	} else {
...
bool Pacer::renderOnMainThread(std::shared_ptr<VideoRenderer> &sceneRenderer) {
	...
	if (IsXboxOne()) {
		return renderModeCadence(sceneRenderer);
	} else {
		return renderModeDirect(sceneRenderer);
	}
}
...
bool Pacer::renderModeDirect(std::shared_ptr<VideoRenderer> &sceneRenderer) {
...
bool Pacer::renderModeCadence(std::shared_ptr<VideoRenderer> &sceneRenderer) {
	...
	// Render it
	FQLog("> Frame rendered [pts: %.3f] [%.2ffps] [%.2fhz] [fpv %.2f] [advanceCount %d] [queued %d]\n",
	      m_CurrentFrame->pts / 90.0, m_FrameCadence.streamFps(), m_FrameCadence.displayHz(),
	      m_FrameCadence.framesPerVblank(), advanceCount, queueDepth);
