...
void moonlight_xbox_dxMain::StartRenderLoop()
{
	...
	auto workItemHandler = ref new WorkItemHandler([this](IAsyncAction ^ action) {
		...
		while (action->Status == AsyncStatus::Started) {
			...
			bool waitFullTime = true; // keep waiting the full wait time after a frame is available
			Pacer::instance().waitForFrame(maxWaitMs, waitFullTime);

			{
...

				{
					...
					if (!Render()) {
						// we're receiving a lower framerate so no frame is available to present,
						// but we still want to wait the rest of this vblank interval
						Pacer::instance().waitBeforePresent(deadline);
						continue;
					}
					t2 = QpcNow();

					// final small wait to target vblank. We want to minimize time in this block
					// because it locks the ffmpeg decoder thread, but we also must avoid missing deadline
					Pacer::instance().waitBeforePresent(deadline);
